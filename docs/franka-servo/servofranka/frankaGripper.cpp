...
int main(int argc, char **argv)
{
  ...
  double opt_max_grip = 0.;
  double opt_grasping_force = 0.1;
  ...
  for (int i = 1; i < argc; i++) {
    ...
    } else if (std::string(argv[i]) == "--grasp" && i + 2 < argc) {
      opt_gripper_state = Gripper_Grasp;
      opt_grasping_width = std::atof(argv[i + 1]);
      opt_grasping_force = std::atof(argv[i + 2]);
    } else if (std::string(argv[i]) == "--test"  && i + 1 < argc) {
      opt_gripper_state = Gripper_Test;
      opt_grasping_width = std::atof(argv[i + 1]);
    } else if (std::string(argv[i]) == "--move" && i + 1 < argc) {
      opt_gripper_state = Gripper_Move;
      opt_grasping_width = std::atof(argv[i + 1]);
    } else if (std::string(argv[i]) == "--random" && i + 1 < argc) {
      opt_gripper_state = Gripper_Random;
      opt_grasping_width = std::atof(argv[i + 1]);
    } else if (std::string(argv[i]) == "--drop" && i + 2 < argc) {
      opt_gripper_state = Gripper_Drop;
      opt_grasping_width = std::atof(argv[i + 1]);
      opt_max_grip = std::atof(argv[i + 2]);
    }
  ...
  if (opt_gripper_state == Gripper_None) {
    ...
    return EXIT_SUCCESS;
  } else if ((opt_gripper_state == Gripper_Grasp || opt_gripper_state == Gripper_Test || opt_gripper_state == Gripper_Move || opt_gripper_state == Gripper_Random || opt_gripper_state == Gripper_Drop) && opt_grasping_width <= 0) {
  ...
  try {
    ...
    if (opt_gripper_state == Gripper_Home) {
      ...
      std::cout << opt_grasping_force << std::endl;
      robot.gripperGrasp(opt_grasping_width, opt_grasping_force);
    }
    ...
    else if (opt_gripper_state == Gripper_Move) {
      std::cout << "- Gripper move to " << opt_grasping_width << "m.." << std::endl;
      robot.gripperMove(opt_grasping_width);
    }
    else if (opt_gripper_state == Gripper_Random) {
      // Add some noise to the grasping-width
      std::default_random_engine generator;
      generator.seed(time(NULL));
      std::normal_distribution<double> distribution(0, 5e-5);
      double noise = distribution(generator);
      opt_grasping_width += noise;

      // Grasp object
      opt_grasping_width = std::ceil(opt_grasping_width * 100000.0) / 100000.0;
      std::cout << "- Gripper random " << opt_grasping_width << "m.." << std::endl;
      robot.gripperMove(opt_grasping_width);
    }
    else if (opt_gripper_state == Gripper_Drop) {
      std::cout << "Dropping object.." << std::endl;
      for (double i = opt_grasping_width; i < opt_max_grip; i+=0.0001) {
        robot.gripperMove(i);
      }
    }
